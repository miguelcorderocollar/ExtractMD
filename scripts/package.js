...
async function checkSourceMaps() {
  ...
  try {
    ...
    if (sourceMaps.length > 0) {
      warnings.push(`Source maps found in dist/: ${sourceMaps.join(', ')}. These should not be included in production ZIP.`);
...
async function createZip() {
  ...
  try {
    ...
    try {
      ...
      log('System zip command not available or failed. Providing manual instructions...', 'warning');
  ...
  try {
    ...
    if (manifest.background?.service_worker) {
      ...
      if (!(await fileExists(bgPath))) {
        errors.push(`Manifest references missing background script: ${manifest.background.service_worker}`);
      }
    }
